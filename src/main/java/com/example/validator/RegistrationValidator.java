...
public class RegistrationValidator {
    ...
    public void validate(@Size(max = 30) User user, BindingResult result) {
        ...
        if (user.getOthesctvo().isEmpty()) {
            result.rejectValue("othesctvo", null, "Поле 'Отчество' не может быть пустым!");
        }

        if (user.getOthesctvo().matches(".*\\d.*")) {
            result.rejectValue("othesctvo", null, "Поле 'Отчество' не может содержать в себе цифры");
        }
